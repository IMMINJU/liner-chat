...
- max_tokens: 2000 (4000 → 3000 → 2000으로 두 번 줄임 — 당시 plain Vercel Hobby 60s 함수 한도 안에 응답이 들어오도록). ※ 이후 Fluid Compute로 함수 한도가 300s로 올라가 이 압박은 완화됐다(타임아웃 계층은 CLAUDE.md 참조). 2000은 품질·비용 균형으로 유지 중이며, 한도가 풀렸다고 자동으로 다시 올릴 필요는 없다.
...
>
> 10. **검증 통과 (메타데이터 정확도).** 제출한 곡은 Spotify Search 검증(artist 정확 일치 + album 부분 일치 + 발매연도 ±2)을 통과해야 사용자에게 노출된다. **이 원칙은 선곡을 바꾸지 않는다** — 여전히 deep cut을 고르되(원칙 2), 고른 곡의 **표기만** Spotify 정규판 기준으로 정확히 적는다: track은 리마스터 부제 떼고, artist는 메인(첫) 아티스트 한 명만, album은 처음 실린 정규 스튜디오 앨범명(베스트/컴필 금지), year는 오리지널 발매연도(리마스터 연도 아님). 드랍의 원인은 곡이 묻혀서가 아니라 표기가 틀려서이므로, deep cut 자유와 충돌하지 않는다. (이 지침은 원래 보충 호출에만 있었으나 1차 호출의 검증 통과율을 올려 보충 호출 자체를 줄이려고 1차 프롬프트로 끌어올림.)
>
> **카테고리별 최소 개수:**
